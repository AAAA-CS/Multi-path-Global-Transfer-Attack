...
def main():
    ...
    model = SwinTransformer(patch_size=windowSize_max, in_chans=b, num_classes=Classes)
    ...
    for epoch in range(args.epochs):
        ...
        for step, (b_x0,b_y) in enumerate(train_loader):  # gives batch data, normalize x when iterate train_loader
            ...
            torch.save(model.state_dict(), (args.saveNet_path + 'SwinTransformer.pkl'))
            model.train()

    model.load_state_dict(torch.load(args.saveNet_path + 'SwinTransformer.pkl'))
    model.eval()
